...
#
#
# Version: 2.56     Last Update Date: 3/18/2017
#
...
# Some packages can be nosiy and not important for analysis. Use this to skip
# them in APKSmash. This match happens on the file directory name or the
# package, so use '\\' instead of '.'
...
searchterms = {
    'android/content/Context;->openFileOutput': 'OpenFile',
...
def get_var_from_line(line, varnumber):
    """
    Function (get_var_from_line) returns which string of the smali variable at a
    certain  position in the line. This can be useful if we later want to log out
    that variable. Note, if a method uses over 16 registers, it will typically
    need to swap things and use a virtual range. This causes me headaches and
    ...
    # Cheating here. There maybe more vars, but this should work since the
    # list starts with " {"
    myregexstring = ' \{'
